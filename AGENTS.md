...
优先级：直观 > 简单。适用于内容画面（课程场景、图表组件）；交互控件只受可用性规范（触控热区、图标语义）。修改前逐条自查：
...
- 验收分工：几何/数量/颜色类由断言强制；直观性/语义类无法断言，以用户目检为准——agent 提交前跑机械项并附「待目检清单」（须涵盖所有阶段/视图切换无瞬切）
...
- **SVG path d 过渡**：路径点数归一化后（同命令数、同命令类型），CSS `d` 属性过渡在 Chrome/Edge/Firefox 自动生效；Safari 不支持（WebKit 性能问题搁置），采用渐进增强——设计时确保每帧路径变化幅度小，使不支持 `d` 过渡的浏览器也能完整呈现该帧的目标形态
